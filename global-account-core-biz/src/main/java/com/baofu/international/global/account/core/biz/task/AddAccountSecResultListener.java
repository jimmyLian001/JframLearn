...
@Slf4j
@Service
public class AddAccountSecResultListener implements MessageListener {
    ...
    public void onMessage(Message message) {
        ...
        try {
            ...
            log.info("收到收款账户开户结果渠道第二次异步通知MQ，队列名称：{}，消息内容：{}", binding, msgText);
            ...
            channelNotifyApplyStatusBiz.updateApplyAccountInfo(PayeeAccountConvert.paramConvert(cgwLookUserResp));
        } catch (Exception e) {
            log.error("收到收款账户开户结果渠道第二次异步通知MQ，处理消息失败 Exception:{}", e);
        }
    }
}
