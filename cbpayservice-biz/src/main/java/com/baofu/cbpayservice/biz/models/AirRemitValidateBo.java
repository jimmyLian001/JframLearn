...
@Setter
@ToString
public class AirRemitValidateBo {
    ...
    @DecimalMax(value = Constants.MAX_QUATO, message = "订单金额不能大于99999999999999.99")
    ...
    @Pattern(regexp = "[^；]*", message = "航班号不能含有中文分号")
    private String flightNumber;

    /**
     * 起飞时间
     */
    @NotBlank(message = "起飞时间不能为空")
    @Pattern(regexp = "[^；]*", message = "起飞时间不能含有中文分号")
    private String takeoffTime;

    /**
     * 起飞地目的地
     */
    @NotBlank(message = "起飞地目的地不能为空")
    @Pattern(regexp = "[^；]*", message = "起飞地目的地不能含有中文分号")
