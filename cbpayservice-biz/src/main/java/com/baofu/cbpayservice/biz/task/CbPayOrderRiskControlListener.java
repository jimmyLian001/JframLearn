...
@Slf4j
@Service
public class CbPayOrderRiskControlListener implements MessageListener {
    ...
    public void onMessage(Message message) {
        ...
        try {
            ...
            log.info("call 购付汇 收到跨境订单风控请求MQ，队列名称：{}，消息内容：{}", binding, msgText);
            String replace = msgText.replace("\"", "").replace("[", "").
                    replace("]", "");
            String[] strings = replace.split(",");
            List<String> batchNos = Arrays.asList(strings);
            cbpayOrderRiskControlBiz.handelOrderRiskControl(batchNos);
        } catch (Exception e) {
