 ...
 */
@Slf4j
public abstract class BaseIDManager {
     ...
     * @return 19位id
     */
    protected String generate19By(String header, IDType idType, String redisKeyName, long redisMaxValue,
                                  String luckNo) {

        //redis计数取值
        long redisSeq = redisManager.incr(redisKeyName);
        if (redisSeq > redisMaxValue) {
            redisManager.deleteObject(redisKeyName);
        }
        ...
        //拼装客户编号，验证位用X代替
        StringBuilder idSb = new StringBuilder().append(header).append(idType.getCode());
        ...
        log.info("创建{}为:{}", idType.getDesc(), idStr);
        return idStr;
    }
