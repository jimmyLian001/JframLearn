        ...
        $.ajax({
            ...
            success: function(data){
                ...
                for (var i = 0; i < provinceList.length; i++) {
                    if (provinceName == provinceList[i].provinceName) {
                        selector.append(
                            "<option value='" + provinceList[i].provinceId + "_" + provinceList[i].provinceName + "' selected>"
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                for (var i = 0; i < cityList.length; i++) {
                    if (initFlag && cityName == cityList[i].cityName) {
                        cityId = cityList[i].cityId;
                        selector.append("<option value='" + cityList[i].cityId + "_" + cityList[i].cityName +"' selected>" + cityList[i].cityName + "</option>");
                    } else {
    ...
    function initArea(cityId) {
        ...
        var areaName = $("select[name='area']").val();
        var params = [
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                for (var i = 0; i < areaList.length; i++) {
                    if (initFlag && areaName == areaList[i].areaName) {
                        selector.append("<option value='" + areaList[i].areaId + "_" + areaList[i].areaName +"' selected>" + areaList[i].areaName + "</option>");
                    } else {
