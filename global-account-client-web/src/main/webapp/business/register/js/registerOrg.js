...
function sendEmail(thisId) {
    ...
    if (!checkEmail(idLoginNo)) {
        return;
    }
    $.ajax({
        url: ctx + "/register/ajaxSendEmailCaptcha.do",
        type: "post",
...
function nextSubmit() {
    ...
    $.ajax({
        cache: true,
        url: ctx + "/register/ajaxCheckEmailCaptcha.do",
        type: "POST",
        data: {
            loginNo: $('#loginNo').val(),
            captcha: $('#captcha').val(),
        ...
        success: function (data) {
            if (data.code == 0) {
                ...
                $("#regOrgFrom").find("input[type='text']").val("");
                $("#regOrgFrom").find("input[type='password']").val("");
                $("#nextForm").submit();
            }
            $('#nextSubmit').attr("disabled", false);
        },
...
function submitCreate() {
    ...
    $.ajax({
        cache: true,
        url: ctx + "/register/ajaxCreateOrgUser.do",
        type: "POST",
        data: $("#orgQuestionForm").serialize(),
        async: false,
        success: function (data) {
            if (data.code == 0) {
                topTip(data.message);
